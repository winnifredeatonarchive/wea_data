...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="wea:appendMLA" as="item()+">
        ...
        <xsl:text> </xsl:text><title level="m">The Winnifred Eaton Archive</title>, edited by <editor>Mary Chapman</editor> and <editor>Jean Lee Cole</editor>, <edition n="{$version}">v. <xsl:value-of select="$version"/></edition>, <date when="{$today}"><xsl:value-of select="format-date(current-date(),'[D01] [MNn] [Y0001]')"/></date>, <ref target="{$uri}"><xsl:value-of select="$uri"/></ref><xsl:text>.</xsl:text>
    </xsl:function>
